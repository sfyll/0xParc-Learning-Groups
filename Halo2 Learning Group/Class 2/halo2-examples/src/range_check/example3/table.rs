...
impl<F: FieldExt, const NUM_BITS: usize, const RANGE: usize> RangeTableConfig<F, NUM_BITS, RANGE> {
    ...
    pub(super) fn load(
        ...
        layouter.assign_table(||"load range check table", |mut table| {
            ...
            table.assign_cell(||"assign num_bits = 1", 
            self.num_bits, 
            offset, 
            ||Value::known(F::one()))?;

            table.assign_cell(||"assign value = 0", 
            self.value, 
            offset, 
            ||Value::known(F::zero()))?;
            ...
            for num_bits in 1..=NUM_BITS {
                for value in 1 << ( num_bits - 1)..(1 << num_bits) {
                    ...
                    table.assign_cell(||"assign num_bits", 
                    self.num_bits, 
                    offset, 
                    ||Value::known(F::from(num_bits as u64)))?;

                    table.assign_cell(||"assign value", 
                    self.value, 
                    offset, 
